...
export async function GET(req: NextRequest, ctx?: { params?: Promise<any> }) {
  ...
  try {
    const { data, error } = await db.from('warranties').select('*').eq('user_id', user.id).order('created_at', { ascending: false })
...
export async function POST(req: NextRequest, ctx?: { params?: Promise<any> }) {
  ...
  try {
    ...
    const { data, error } = await db.from('warranties').insert(payload).select()
    if (error) return NextResponse.json({ success: false, error: error.message }, { status: 500 })
    return NextResponse.json({ success: true, data })
  } catch (err: any) {
    return NextResponse.json({ success: false, error: err.message }, { status: 500 })
