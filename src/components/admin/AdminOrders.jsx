...
export default function AdminOrders() {
  ...
  const normalizeOrder = (order) => ({
    ...
    customer: order.customer || order.customer_info || order.user || { name: order.email || 'Customer', email: order.email },
    createdAt: order.created_at || order.createdAt || null,
    fulfillmentStatus: (order.fulfillmentStatus || order.status || order.fulfillment_status || 'pending').toString().toLowerCase(),
    paymentStatus: order.payment_status || order.paymentStatus || 'unpaid',
    items: (order.items || order.line_items || []).map((item) => {
      const quantity = Number(item.quantity || item.qty || 1)
      const unitPrice = Number(item.price || item.unit_price || item.amount || 0)
      return {
        ...item,
        name: item.name || item.title || item.product_name || item.product?.title || item.product?.name || 'Item',
        quantity,
        total: Number(item.total || unitPrice * quantity),
      }
    }),
    subtotal: Number(order.subtotal || order.totalAmount || order.total || 0),
    total: Number(order.total || order.totalAmount || order.subtotal || 0),
    shippingAddress: formatShippingAddress(order.shipping_address || order.shippingAddress || ''),
  ...
  const handleDispatchWithGig = async (order) => {
    if (!order?.id) return

    setDispatchingOrderId(order.id)
    try {
      const res = await api.post(`/admin/orders/${order.id}/gig`)
      const updatedOrder = res?.data?.order ? normalizeOrder(res.data.order) : null
      const waybill = res?.data?.waybill || updatedOrder?.tracking_number || ''

      await loadOrders()

      if (selectedOrder?.id === order.id && updatedOrder) {
        setSelectedOrder(updatedOrder)
      }

      showToast?.(
        waybill ? `GIG dispatch created successfully. Waybill: ${waybill}` : 'GIG dispatch created successfully.',
        'success'
      )
    } catch (err) {
      const message =
        ...
        err?.message ||
        'Failed to dispatch with GIG'
      console.error('Failed to dispatch order with GIG', err?.response?.data || err)
  ...
  const summaryCards = [
    ...
    },
    {
      ...
      columnClass: 'sm:col-span-2 xl:col-span-1',
    },
  ];
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-3">
          {summaryCards.map((card) => {
            ...
            return (
              <div
                key={card.label}
                className={`min-w-0 rounded-2xl border border-slate-200 bg-white/90 p-5 shadow-sm transition-shadow hover:shadow-md sm:p-6 ${card.columnClass || ''}`}
              >
                <div className="flex items-start justify-between gap-4">
                  <div className="min-w-0">
                    <p className="text-sm font-semibold tracking-tight text-slate-500">{card.label}</p>
                    <p className={`mt-5 font-bold tracking-tight text-slate-900 tabular-nums ${card.valueClass || 'text-3xl sm:text-4xl'}`}>
                      {card.value}
                    </p>
                  </div>
                  <div className={`flex h-16 w-16 shrink-0 items-center justify-center rounded-2xl ${card.iconClass}`}>
              ...
              <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
                <div className="overflow-x-auto overscroll-x-contain">
                  <table className="w-full min-w-[1080px] table-fixed">
                    <thead>
                      <tr className="bg-gradient-to-r from-purple-50 to-indigo-50 [&>th]:whitespace-nowrap">
                        <th className="w-[19%] py-3 px-4 text-left font-semibold text-slate-700 text-sm">Order #</th>
                        <th className="w-[23%] py-3 px-4 text-left font-semibold text-slate-700 text-sm">Customer Email</th>
                        <th className="w-[13%] py-3 px-4 text-left font-semibold text-slate-700 text-sm">Total</th>
                        <th className="w-[14%] py-3 px-4 text-left font-semibold text-slate-700 text-sm">Order Status</th>
                        <th className="w-[11%] py-3 px-4 text-left font-semibold text-slate-700 text-sm">Payment Status</th>
                        <th className="w-[12%] py-3 px-4 text-left font-semibold text-slate-700 text-sm">Date</th>
                        <th className="w-[8%] py-3 px-4 text-left font-semibold text-slate-700 text-sm">Actions</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-200">
                      {filteredOrders.length === 0 ? (
                        ...
                        filteredOrders.map((order) => {
                          const statusInfo = getStatusInfo(order.fulfillmentStatus || order.status);
                          const StatusIcon = statusInfo.icon;
                          const placedAt = formatDateParts(order.createdAt);
                          
                            return (
                            <tr key={order.id} onClick={() => setSelectedOrder(order)} className="transition-colors cursor-pointer hover:bg-gray-50">
                              <td className="py-4 px-4 align-top">
                                <div className="block max-w-full truncate font-semibold text-slate-900" title={order.orderNumber}>
                                  {order.orderNumber}
                                </div>
                              </td>
                              <td className="py-4 px-4 align-top">
                                <div className="block max-w-full truncate text-sm text-slate-700" title={order.customer?.email || order.customer?.name || '—'}>{order.customer?.email || order.customer?.name || '—'}</div>
                              </td>
                              <td className="py-4 px-4 align-top">
                              ...
                              <td className="py-4 px-4 align-top">
                                <div className={`px-3 py-1 rounded-full ${statusInfo.bgColor} ${statusInfo.textColor} text-xs font-medium inline-flex items-center gap-2 whitespace-nowrap`}>
                                  <StatusIcon size={12} />
                                  {(order.fulfillmentStatus || order.status || '').toString().charAt(0).toUpperCase() + (order.fulfillmentStatus || order.status || '').toString().slice(1)}
                                </div>
                              </td>
                              <td className="py-4 px-4 align-top">
                                <div className="text-sm font-medium text-slate-900 whitespace-nowrap capitalize">{order.paymentStatus || order.__raw?.payment_status || '—'}</div>
                              </td>
                              <td className="py-4 px-4 align-top">
                                <div className="flex items-start gap-2 text-slate-700 text-sm">
                                  ...
                                  <div className="flex flex-col whitespace-nowrap leading-tight">
                                    ...
                                    {placedAt.time && <span className="mt-1 text-xs text-slate-500">{placedAt.time}</span>}
                                  </div>
                                </div>
                              </td>
                              <td className="py-4 px-4 align-top">
                                <button onClick={(e) => { e.stopPropagation(); setSelectedOrder(order); }} className="inline-flex items-center gap-2 px-3 py-1 bg-white/10 hover:bg-white/20 rounded text-sm whitespace-nowrap">
          ...
          <div className="bg-white rounded-xl sm:rounded-2xl lg:rounded-3xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            <div className="p-4 sm:p-6 lg:p-8">
              <div className="flex justify-between items-center mb-4 sm:mb-6">
                ...
                {(() => {
                  const status = (selectedOrder.fulfillmentStatus || selectedOrder.status || '').toLowerCase()
                  const canDispatch = status !== 'shipped' && status !== 'delivered'
                  return canDispatch ? (
                    <button
                      onClick={() => handleDispatchWithGig(selectedOrder)}
                      disabled={dispatchingOrderId === selectedOrder.id}
                      className="px-3 py-2 rounded-md bg-emerald-600 text-white text-sm disabled:opacity-60"
                    >
                      {dispatchingOrderId === selectedOrder.id ? 'Dispatching...' : 'Dispatch with GIG'}
                    </button>
                  ) : null
                })()}
                <div className="flex items-center gap-2">
